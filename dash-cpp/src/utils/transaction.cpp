...
namespace dash
{
    ...
    T_InputType Transaction::type_ = T_InputType::normal; // 默认为普通输入
    ...
    void Transaction::transactionStart(const std::string &transaction_name){
        ...
        if (transaction_map_.find(transaction_name) != transaction_map_.end()) {
            ...
            setT_InputType(T_InputType::transaction); // 设置为事务开始
    ...
    void Transaction::transactionRecord(const std::string &transaction_name){
        ...
        setT_InputType(T_InputType::record); // 设置为记录输入
    ...
    void Transaction::transactionComplete() {
        ...
        setT_InputType(T_InputType::normal); // 恢复为普通输入
    ...
    T_InputType Transaction::getT_InputType( bool update){ 
        //更新一下可能出错的地方
        if (update && type_ == T_InputType::special){
            if (special_command_list_.size() == 0){
                setT_InputType(T_InputType::normal);
            }
        }
        return type_; 
    }

    int Transaction::setT_InputType(T_InputType type){ type_ = type; return 1;}
