...
void History::addCommand(const std::string& command) {
    ...
    // 同步命令到readline历史记录
#ifdef READLINE_ENABLED
    // 检查命令是否已在readline历史记录中
    HIST_ENTRY* last_entry = history_get(history_length - 1);
    if (!last_entry || std::string(last_entry->line) != command) {
        // 如果不在readline历史记录中，则添加
        add_history(command.c_str());
        DebugLog::logCommand("命令已同步到readline历史记录: " + command);
    }
#endif
}
...
#ifdef READLINE_ENABLED
    clear_history();
    DebugLog::logCommand("readline历史记录已清除");
#endif
}
...
bool History::loadFromFile(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        if (spacePos != std::string::npos) {
            try {
                ...
                // 同步到readline历史记录
#ifdef READLINE_ENABLED
                add_history(entry.command.c_str());
#endif
    ...
    DebugLog::logCommand("从文件加载了 " + std::to_string(history_.size()) + " 条历史记录: " + filename);
    return true;
}
