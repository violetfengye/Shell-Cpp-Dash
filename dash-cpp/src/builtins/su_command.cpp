...
namespace dash
{
    ...
    std::string SuCommand::dash_path = "";
    SuCommand::SuCommand(Shell *shell)
        : BuiltinCommand(shell)
    {
        // 获取工作目录
        dash_path = getcwd(nullptr, 0);
        dash_path += "/dash";
        // 设置工作目录
    }
    ...
    int SuCommand::execute(const std::vector<std::string> &args)
    {
        ...
        if (mode == 1){
            finalCommand = "su - " + userName + " -c \"" +dash_path + "\"";
        }else{
            finalCommand = "su - " + userName + " -c \"" +commandText + "\"";
        }
        Transaction::addSpecialCommand(finalCommand);
        Transaction::setT_InputType(T_InputType::special);
