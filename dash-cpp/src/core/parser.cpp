...
namespace dash
{
    ...
    {
        alias_manager_ = std::unique_ptr<AliasManager> (new AliasManager(*shell));
    }
    ...
    std::unique_ptr<Node> Parser::parseCommand(bool interactive)
    {
        try
        {
            ...
            if (interactive)
            {
                ...
                switch (Transaction::getT_InputType())
                {
                //特殊的事务，用于实现su指令的调用
                case T_InputType::special:
                    line = Transaction::getSpecialCommand();
                    if (line != "")break;
                    else {
                        Transaction::setT_InputType(T_InputType::normal);
                    }
                case T_InputType::normal:
                    // 从 shell 的输入处理器获取一行输入
                    line = shell_->getInput()->readLine(true);
                    break;
                case T_InputType::record:
                    // 从 shell 的输入处理器获取一行输入
                    line = shell_->getInput()->readLine(true);
                    // 传入事务处理器
                    Transaction::addCommandString(line);
                    break;
                case T_InputType::transaction:
                ...
                // 解析别名
                line = alias_manager_->expandAlias(line);
